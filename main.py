...
class LiveCount(webapp2.RequestHandler):
    def get(self):
        ...
        template = JINJA_ENVIRONMENT.get_template('live-count.html')
...
class SongRankingDo(webapp2.RequestHandler):
    def post(self):
        albums = self.request.get_all("albums")
        songs = []

        for album in albums:
            
            for record in NukotanSong().query(NukotanSong.album == album):
                songs.append(record.title)
        
        ktop = self.request.get("ktop")
        algorithm = self.request.get("algorithm");
        template = JINJA_ENVIRONMENT.get_template('song_ranking_do.html')
        template_values = {'songs': songs, 'ktop': ktop, 'algorithm': algorithm}
